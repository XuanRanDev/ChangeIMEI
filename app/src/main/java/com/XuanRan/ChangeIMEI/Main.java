    ...
    public void handleLoadPackage(final XC_LoadPackage.LoadPackageParam loadPackageParam) throws Throwable {
        /*XposedHelpers.findAndHookMethod(XposedBridge.class, "log", String.class, new XC_MethodHook() {
            ...
            protected void beforeHookedMethod(MethodHookParam param) throws Throwable {
                ...
                //Log.i("ChangeIMEI", param.args[0].toString());
            }
        });*/
        ...
        if (packagename.equals("com.XuanRan.ChangeIMEI") || packagename.equals("com.taobao.taobao") || packagename.equals("com.example.xiejun.xposedstudy.app")) {
           ...
           // new AutoCallHookl().handleLoadPackage(loadPackageParam);

            XposedBridge.log("start hook app is " + loadPackageParam.packageName);

            new DuiZhanPrint().handleLoadPackage(loadPackageParam);



            //  ChangeIMEI(loadPackageParam);

           // CheckSaveDeviceInfo(loadPackageParam);

           // new PhoneInfoHook().handleLoadPackage(loadPackageParam);


        }
    }

    private void CheckSaveDeviceInfo(XC_LoadPackage.LoadPackageParam loadPackageParam) {

        final Class<?> clazz = XposedHelpers.findClass("mtopsdk.mtop.deviceid.DeviceIDManager", loadPackageParam.classLoader);
        XposedHelpers.findAndHookMethod(clazz, "saveDeviceIdToStore", Context.class, String.class, String.class, String.class ,new XC_MethodHook() {

            @Override
            protected void beforeHookedMethod(final MethodHookParam param) throws Throwable {
                super.beforeHookedMethod(param);
                XposedHelpers.setStaticObjectField(clazz, "$ipChange", null);
                XposedBridge.log("^^^^^^^^^^^^saveDeviceIdToStore被调用^^^^^^^^^^^^^^^^^^^^^");
                XposedBridge.log(String.format("%s|%s|%s",param.args[1],param.args[2],param.args[3]));
            }
        });
    }

    private void ChangeIMEI(final XC_LoadPackage.LoadPackageParam loadPackageParam) {
